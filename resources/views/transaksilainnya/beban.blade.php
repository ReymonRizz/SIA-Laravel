    ...
    <table class="table table-bordered" id="datatable">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><button class="btn btn-warning" onclick="editBeban('{{ $data }}')"><i class="fas fa-edit"></i> Edit Beban</button>&nbsp;<button class="btn btn-danger" onclick="deleteBeban('{{ $data->id_beban }}')"><i class="fas fa-trash"></i> Hapus Beban</button></td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<!-- modal Add Beban -->
<div class="modal fade" id="addBebanModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <form action="/data-beban/add" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="serba_serbi" class="form-control" required id="serba_serbi_add">
...
<!-- modal Edit Beban -->
<div class="modal fade" id="editBebanMdl" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group mb-2">
                            ...
                            <input type="text" class="form-control price" name="nominal_format" id="nominal_format_edit" />
                            <input type="hidden" name="nominal" id="nominal_edit" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Keterangan</label>
...
<script>
    ...
    $('#serba_serbi_add').change(function(ev) {
        ev.preventDefault()
        if ($(this).val().includes('Beban Penyusutan')) {
            $.ajax({
                url: 'data-beban/getNominal',
                type: 'GET',
                cache: false,
                success: function(res) {
                    res = formatNumber(res)
                    $('#nominal_format').val(res)
                    res = res.toString().replace(/\./g, '')
                    $('#nominal').val(res)
                    $('#nominal_format').attr('readonly', true)
                }
            })

        } else {

            $('#nominal_format').attr('readonly', false)
            $('#nominal_format').val('')
            $('#nominal').val('')
        }
    })
